module Card
  ...
  def get_card(value, suit)
      ...
      value = value.to_i rescue CardError.new("Card value #{value} is invalid")

      if not SUITS[suit]
        raise CardError.new("#{suit} is not a valid suit.")
      ...
      raise CardError.new("#{value} not valid for #{suit} suit.") unless card.valid_value?
  ...
  class Card
    ...
    def self.point_lookup()
      @point_lookup
    end

    def self.add_name(value, name)
      @name_lookup = {} unless @name_lookup
      @name_lookup[value] = name
    end

    def self.add_points(value, points)
      @point_lookup = {} unless @point_lookup
    ...
    def get_points
      self.class.point_lookup[@value] ? self.class.point_lookup[@value] : 1
    end
